...
(use-existing-pkg consult
  :init
  (progn
    (mapcar (lambda (x) (advice-add x :around #'vertico--advice))
            '(consult--line consult--line-multi-candidates consult--grep consult--read))
    (progn
      (advice-add #'consult--line :around #'vertico--advice)
      (advice-add #'consult--line-multi-candidates :around #'vertico--advice))
    (progn
