...
(let ((option-file-name "~/.emacs.d/config/options.txt"))
  ...
  (defvar dhnam/machine-options (car (read-from-string (get-string-from-file option-file-name)))))
...
(setq dhnam/machine-config-list
      '((theme-style (white-theme-machine white))
        ...
        (installable-packages
         ...
         (base (vlf magit dash multi-term counsel async wgrep which-key paredit poporg ctrlf expand-region company vterm clipetty))
         (documentation (valign org-fragtog auctex latex-preview-pane pdf-tools define-word pdfgrep biblio))
